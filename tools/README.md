...
`carve_hill_mine.py`：读取保留的 `sources/environment/island-coast-source.blend`，从西山地表切出入口与沟槽，并生成岩壁；只向 `--` 指定的绝对临时目录输出 `island-mine-source.blend`、分块 `island.glb` 和制作清单。检查后分别安装到 `sources/environment/` 与 `assets/models/`，不覆盖保留的海岸阶段源文件。
